...
class SimpanHeader extends Rest_Controller {

	function __construct() {
		parent::__construct();
		$this->load->database();
		$this->load->model('M_simpanheader');
		}

		public function index_get() {
			//$docEntry = $this->get('docEntry');
      $workCenter = $this->get('workCenter');
			$data = $this->M_simpanheader->read( $workCenter);
			if ($data) {
				# code...
				$this->response([
					'status' => TRUE,
       ...
       $cek = $this->M_simpanheader->addHeader($data);
       if ($cek) {
       	$this->response([
       		'status' => TRUE,
